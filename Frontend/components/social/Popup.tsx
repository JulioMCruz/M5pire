  ...
  return (
    ...
    >
      <div className="relative w-full max-w-[500px] rounded-lg bg-white p-6">
        ...
        <h2 className="mb-4 text-xl font-semibold text-black">{comments.length} comments</h2>

        <div className="scrollbar-none mb-4 max-h-60 overflow-y-auto">
          {comments.length > 0 ? (
            <ul className="space-y-2">
              {comments.map((c, index) => (
                <li key={index} className="border-b border-gray-200 p-2 text-black">
                  <div className="flex items-center gap-3">
                    <CircleUserRound color="#4f4f4f" />
                    <span className="font-semibold text-black">Alex</span>
                  </div>
                  {c}
                </li>
              ))}
